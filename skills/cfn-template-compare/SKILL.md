...
Agent:
1. Retrieves deployed template from example-st
...
```
cfn-compare-results/
  {date}_{time}_deployed-{branch}_local-{branch}_ref-{deploy_reference}/
...
- **DEPLOY_REFERENCE**: Environment identifier from env-local.mk

**Example**:

```
2026-02-18-153045_deployed-main_local-feat-PROJ-990-clean-subscription-fix_ref-user/
```
...
```
Error: Stack 'example-monitoring-st' not found in region eu-west-1
...
# 2. Run comparison
User: "Compare my local CDK template with deployed example-monitoring-st stack"
