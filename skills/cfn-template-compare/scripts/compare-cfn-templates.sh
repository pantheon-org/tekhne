...
#
# Example:
#   compare-cfn-templates.sh example-monitoring-st eu-west-1 example-st
#   compare-cfn-templates.sh example-monitoring-st eu-west-1 example-st cdk.out/my-stack.template.json
#
# Exit codes:
...
DEPLOY_REF=$(grep -E "^export DEPLOY_REFERENCE=" env-local.mk 2>/dev/null | cut -d'=' -f2 | tr -d ' ' || echo "none")

# Working directory for comparison files (timestamped with context)
WORK_DIR="cfn-compare-results/${TIMESTAMP}_deployed-main_local-${CURRENT_BRANCH}_ref-${DEPLOY_REF}"
mkdir -p "$WORK_DIR"
...
analyze_renamed_resources() {
    ...
    local deployed_normalized=$(echo "$deployed_res" | sed -E 's/examplemonitoringst[a-z0-9]*/examplemonitoringst/' | sed -E 's/[A-F0-9]{16}$//' | sed -E 's/[a-f0-9]{8}$//')
    local local_normalized=$(echo "$local_res" | sed -E 's/examplemonitoringst[a-z0-9]*/examplemonitoringst/' | sed -E 's/[A-F0-9]{16}$//' | sed -E 's/[a-f0-9]{8}$//')
...
EOF
    ...
    if [ "$added" -gt 0 ]; then
        ...
        while IFS= read -r local_res; do
            ...
            local local_base=$(echo "$local_res" | sed -E 's/examplemonitoringst[a-z0-9]*/STACKREF/g' | sed -E 's/[A-F0-9]{4,16}//g' | sed -E 's/[a-f0-9]{8}//g')
            ...
            while IFS= read -r deployed_res; do
                ...
                if [ "$deployed_type" = "$local_type" ]; then
                    ...
                    local deployed_base=$(echo "$deployed_res" | sed -E 's/examplemonitoringst[a-z0-9]*/STACKREF/g' | sed -E 's/[A-F0-9]{4,16}//g' | sed -E 's/[a-f0-9]{8}//g')
...
generate_report() {
    ...
    echo "  - local.json: Local template from branch $CURRENT_BRANCH (ref: $DEPLOY_REF)"
