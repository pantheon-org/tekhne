...
test.describe("Code Block Annotation Testing", () => {
  ...
  test("should load page with syntax-highlighted code blocks", async ({
    page,
  }) => {
  ...
  test("should create annotation on code block", async ({ page }) => {
    ...
    if (annotationCount > 0) {
      ...
      const annotationInCode = await page
        .locator("pre code mark[data-bind-id]")
        .count();
  ...
  test("should allow manual annotation via toolbar", async ({ page }) => {
    ...
    try {
      ...
      console.log(
        "Toolbar did not appear - this is expected if toolbar requires specific selection behavior",
      );
  ...
  test("should remove annotation and restore syntax highlighting", async ({
    page,
  }) => {
  ...
  test("should handle multiple annotations on same code block", async ({
    page,
  }) => {
    const codeBlock = page.locator("pre code").first();
    await codeBlock.scrollIntoViewIfNeeded();
    ...
    const annotationsInCode = await page
      .locator("pre code mark[data-bind-id]")
      .count();
    console.log(
      `Multiple annotations test: found ${annotationsInCode} annotations`,
    );
    ...
    const report = {
      ...
      syntaxHighlighting: await page.evaluate(() => {
        ...
        return blocks.map((block, i) => ({
          index: i,
          hasHighlighting:
            block.querySelectorAll('span[class*="hljs"]').length > 0,
      ...
      annotations: await page.evaluate(() => {
        const marks = Array.from(
          document.querySelectorAll("mark[data-bind-id]"),
        );
