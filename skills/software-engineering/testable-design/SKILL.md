...
description: Design code for testability using boundary isolation, dependency injection, and testable architecture patterns. Use when designing test strategies, improving test coverage, or refactoring untestable code.
---
...
Tests are a first-class architectural concern, not an afterthought. Good architecture makes testing easy; bad architecture makes testing impossible.

**Key insight:** If code is hard to test, it's usually a design problem, not a testing problem.
...
- Does this code instantiate concrete dependencies?
- Does it depend on global state (singletons, environment variables)?
- Does it perform side effects (network, file system, database)?
- Is the logic tangled with infrastructure concerns?
