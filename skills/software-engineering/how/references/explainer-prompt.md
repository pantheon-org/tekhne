...
Build the explainer/synthesizer subagent's prompt from this template. Fill in the placeholders.
...
You have access to the codebase to check anything, clarify a detail, or fill a gap. This project has `tokensave` available — use `tokensave_context` (and `tokensave_search`/`tokensave_node` for a specific symbol or file) rather than Read/Grep/Glob; the source sections it returns are the relevant code. The explorers did the heavy lifting, so you shouldn't need to re-explore from scratch.
