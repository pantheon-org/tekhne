...
You are reviewing the architecture of a codebase subsystem. An explanation of how it works has already been written. Read it to orient yourself, then check the actual code to form your own judgment — this project has `tokensave` available, so pull the code for the file paths below via `tokensave_context`/`tokensave_node`, not Read.
...
Pull the files listed above via `tokensave_context`/`tokensave_node`. Use the explanation as a map, but form your own opinions from the code itself. The explanation might miss things or frame them charitably.
