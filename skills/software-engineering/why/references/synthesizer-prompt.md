...
2. Every Direct/Supported claim must have a citation (PR/MR #, ticket ID, doc URL, chat permalink, commit hash, or file:line).
...
2. **Reconcile overlapping findings.** Multiple investigators may have cited the same PR/MR, ticket, or doc. Merge into a single, authoritative reference.
...
- **Source control history**: {file paths}, {number of commits reviewed}, PR/MR #{numbers}, and code comments searched. Or "Not searched. This should not happen because git is always expected."
- **Issue / ticket tracker**: {ticket IDs and keyword searches}. Or "Not searched. No matching MCP available in this session."
- **Long-form documents**: {page titles and search queries}. Or "Not searched. No matching MCP available in this session."
- **Real-time team chat**: {channels searched, date ranges, queries}. Or "Not searched. No matching MCP available in this session."
- **Infrastructure observability**: {dashboards, monitors, metrics, logs, traces, or incidents searched}. Or "Not searched. No matching MCP available in this session."
- **Error / exception tracking**: {issues, events, or releases searched}. Or "Not searched. No matching MCP available in this session."
- **Product analytics warehouse**: {fully-qualified tables queried, the time windows, and the numeric summaries (counts, percentiles, first/last-seen timestamps) that bore on the question}. Or "Not searched. No matching MCP available in this session."

### Confidence Summary

One or two sentences summarizing your overall confidence. E.g.:

> "The core rationale (A) is well-supported by direct PR/MR and ticket evidence. The specific threshold value (100) is inferred from the surrounding context but not explicitly documented. The question of whether this was driven by a customer request could not be answered. No relevant issue tracker or long-form doc content surfaced, and real-time team chat search was unavailable."
