...
Use the Slack MCP tools available in this session. Check the tool schema first if unsure what's available.

1. **Author-bounded search.** Messages from the PR/MR author around the merge date. Limits scope dramatically and often hits gold.
2. **Keyword search for the feature name and key symbols.** Include misspellings and casual phrasings.
3. **PR/MR URL search.** Slack often links PRs/MRs when they're reviewed or discussed. Search for the URL (or just `/pull/<number>` / `/merge_requests/<number>`).
...
- A message from a product manager or customer-facing colleague explaining a customer or regulatory ask
