# Issue / Ticket Tracker (Linear example — adapt for Jira via `mcp-atlassian` or GitLab Issues via `mcp-gitlab`)
...
- Attachments and linked PRs/MRs

This is where the product/business context often lives: the "we're doing this because customer X asked" or "this is for the Q3 compliance initiative" layer. In a regulated domain, watch specifically for regulator-driven motivation — it's a distinct and important category of "why," separate from ordinary product prioritization.

## How to search it

Use whichever ticket-tracker MCP is connected (Jira via `mcp-atlassian`, GitLab Issues via `mcp-gitlab`; adapt tool names accordingly).

1. **Start with linked tickets.** If the seed commits or PRs/MRs reference ticket IDs (e.g., a Jira key like `ENG-1234`, or `[BUG-567]`), fetch those first and read the full issue including comments.
2. **List related issues by keyword.** Search for the feature name, key symbol, or business term. Try multiple phrasings.
3. **Walk the issue tree.** If you land on a sub-issue, fetch its parent (epic/initiative). Sub-issues are tactical; parents often carry the "why."
4. **Read attached docs.** If the issue links a project or epic, check for attached specs. Project-level documents are where specs and rationale are most often captured.
5. **Check labels and milestones/fix-versions.** Labels hint at the category of motivation (customer-request, incident-followup, compliance). Milestones tie work to deadlines, which often reveal motivation.
...
- Labels like `customer:acme`, `incident-followup`, `compliance`, `perf-regression`, or a regulator tag

## Common pitfalls

- **Scope drift.** The ticket the PR/MR references may have been closed and reopened with a different scope. Read the whole history.
...
- Labels, parent issue, project/epic
