...
| --- | --- | --- |
| **SRP** (Single Responsibility) | Does this class have one reason to change? | Multiple concerns → split into focused classes |
| **OCP** (Open-Closed) | Can you extend behavior without modifying existing code? | Adding features requires editing stable code → use abstraction |
| **LSP** (Liskov Substitution) | Do subtypes preserve base behavior contracts? | Subtype breaks parent assumptions → redesign hierarchy |
| **ISP** (Interface Segregation) | Do clients depend only on methods they use? | Clients ignore many interface methods → split interface |
| **DIP** (Dependency Inversion) | Do you depend on abstractions, not concrete details? | Direct concrete coupling → introduce interface/port |
...
### NEVER use god classes with multiple responsibilities

**BAD:** UserService handles auth, persistence, notifications, and reporting.  
**GOOD:** Split into AuthService, UserRepository, NotificationService, ReportingService.

### NEVER modify stable code to add features

**BAD:** Edit PaymentService every time a new processor is added.  
**GOOD:** Define PaymentProcessor interface and add implementations.

### NEVER break base type contracts in subtypes

**BAD:** Subclass throws exception where base type guarantees success.  
**GOOD:** Preserve base type promises or redesign hierarchy.

### NEVER force clients to depend on unused methods

**BAD:** IRepository has 20 methods but clients use only 3.  
**GOOD:** Split into IReadRepository, IWriteRepository, and ISearchRepository.

### NEVER instantiate concrete dependencies in business logic

**BAD:** new PostgresRepository() in OrderService constructor.  
**GOOD:** Depend on IOrderRepository and inject implementation.
