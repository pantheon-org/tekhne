#!/usr/bin/env bash
# shell: bash
# Validate a references/{slug}.md file produced by the triage-paper skill.
# Usage: ./validate-reference-paper.sh <file.md>
set -euo pipefail
...
SCHEMA="$SCRIPT_DIR/../assets/schemas/reference-paper.schema.json"
...
TMP=$(mktemp /tmp/ref-paper-XXXXXX.json)
trap 'rm -f "$TMP"' EXIT
...
check_section "Mechanism overview"
check_section "Evaluation (as reported)"
check_section "Implementation details worth stealing"
