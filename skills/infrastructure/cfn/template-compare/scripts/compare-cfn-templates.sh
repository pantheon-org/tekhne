...
analyze_renamed_resources() {
    ...
    local_normalized=$(echo "$local_res" | sed -E 's/examplemonitoringst[a-z0-9]*/examplemonitoringst/' | sed -E 's/[A-F0-9]{16}$//' | sed -E 's/[a-f0-9]{8}$//')
        ...
        while IFS= read -r local_res; do
            ...
            while IFS= read -r deployed_res; do
                ...
                if [ "$deployed_type" = "$local_type" ]; then
                    ...
                    local deployed_base
                    deployed_base=$(echo "$deployed_res" | sed -E 's/examplemonitoringst[a-z0-9]*/STACKREF/g' | sed -E 's/[A-F0-9]{4,16}//g' | sed -E 's/[a-f0-9]{8}//g')
