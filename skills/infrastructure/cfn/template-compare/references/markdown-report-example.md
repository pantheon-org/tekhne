...
<!-- This file embeds an example generated report (with its own H1 title) and
     shell snippets, so a single-top-level-heading rule does not apply. -->
...
**Deployed Stack**: example-monitoring-st
**Region**: eu-west-1
**Profile**: example-st
**Local Branch**: feat-PROJ-990-clean-subscription-fix
**DEPLOY_REFERENCE: user
...
- **examplemonitoringstuserExampleAccessSecurityGroupfrom1...** (`AWS::EC2::SecurityGroupIngress`)
- **examplemonitoringstuserExampleAccessSecurityGroupfrom2...** (`AWS::EC2::SecurityGroupIngress`)
- **examplemonitoringstuserTicketBaseAnalyticsDailyRuleAllowEventRule...** (`AWS::Lambda::Permission`)
- **examplemonitoringstuserMonitoringMetadataDBFreeStorageSpace15Alarm** (`AWS::CloudWatch::Alarm`)
- **examplemonitoringstuserMonitoringMetadataDBFreeStorageSpace25Alarm** (`AWS::CloudWatch::Alarm`)
- **examplemonitoringstuserMonitoringMetadataDBFreeStorageSpace30Alarm** (`AWS::CloudWatch::Alarm`)
- **examplemonitoringstuserTaskSummaryAnalyticsDailyRuleAllowEventRule...** (`AWS::Lambda::Permission`)

## Removed Resources

The following 7 resource(s) exist in the deployed template but not in local:

- **examplemonitoringstExampleAccessSecurityGroupfrom1...** (`AWS::EC2::SecurityGroupIngress`)
- **examplemonitoringstExampleAccessSecurityGroupfrom2...** (`AWS::EC2::SecurityGroupIngress`)
- **examplemonitoringstTicketBaseAnalyticsDailyRuleAllowEventRule...** (`AWS::Lambda::Permission`)
- **examplemonitoringstMonitoringMetadataDBFreeStorageSpace15Alarm** (`AWS::CloudWatch::Alarm`)
- **examplemonitoringstMonitoringMetadataDBFreeStorageSpace25Alarm** (`AWS::CloudWatch::Alarm`)
- **examplemonitoringstMonitoringMetadataDBFreeStorageSpace30Alarm** (`AWS::CloudWatch::Alarm`)
- **examplemonitoringstTaskSummaryAnalyticsDailyRuleAllowEventRule...** (`AWS::Lambda::Permission`)
...
Attachment: 2026-02-18-153045_deployed-main_local-feat-PROJ-990_ref-user/comparison-report.md

Change Summary:
- 7 resources will be renamed due to DEPLOY_REFERENCE change
...
audit-artifacts/
  2026-Q1/
    2026-02-18-153045_deployed-main_local-feat-PROJ-990_ref-user/
...
bash compare-cfn-templates.sh example-monitoring-st eu-west-1 example-st

# Compare AT environment
bash compare-cfn-templates.sh example-monitoring-at eu-west-1 example-at

# Compare PR environment
bash compare-cfn-templates.sh example-monitoring-pr eu-west-1 example-pr
...
PRESERVE_ARTIFACTS=false bash compare-cfn-templates.sh example-monitoring-st eu-west-1 example-st

# Keep artifacts (default behavior)
PRESERVE_ARTIFACTS=true bash compare-cfn-templates.sh example-monitoring-st eu-west-1 example-st
```
