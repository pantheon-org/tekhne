...
2. Checks for `evals/` directory existence
3. Verifies `.tessl-plugin/plugin.json` exists and checks `private` field value
...
- Agent checks plugin.json for `private: false` setting
...
- Agent publishes without verifying `private: false` in plugin.json
...
- Agent misses any of the three critical requirements (evals, quality, plugin config)
