#!/usr/bin/env sh
# Audit all skills using 9-dimension quality framework
# Usage:
...
**Evaluator:** 9-dimension quality framework (9-dimension evaluation)
...
**Framework:** skill-quality-auditor (9-dimension evaluation)
EOF
}
    ...
    while IFS='|' read -r score skill_name grade _lines _refs; do
      case "$grade" in
        D|F)
          ...
          echo "Run: \`sh ./scripts/evaluate.sh $skill_name --json\` for details" >> "$AUDIT_DIR/remediation-plan.md"
          echo "" >> "$AUDIT_DIR/remediation-plan.md"
          ;;
      esac
    done
...
**Generated by:** skill-quality-auditor  
**Run with:** \`sh ./scripts/generate-remediation-plan.sh --audit-dir $AUDIT_DIR/\`
EOF
}
