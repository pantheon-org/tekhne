...
**Evaluator:** skill-judge framework (9-dimension evaluation)
...
EOF
  ...
  sort -t '|' -k1,1nr "$TMP_DIR/rows.txt" |
    while IFS='|' read -r score skill_name grade lines refs; do
      echo "| $skill_name | $score/140 | $grade | $lines | $refs |" >> "$AUDIT_DIR/analysis.md"
    done
    ...
    sort -t '|' -k1,1nr "$TMP_DIR/rows.txt" |
      while IFS='|' read -r score skill_name grade _lines _refs; do
        if [ "$grade" = "D" ] || [ "$grade" = "F" ]; then
          echo "- **$skill_name** ($score/140) - Requires immediate attention" >> "$AUDIT_DIR/analysis.md"
        fi
      done
      ...
      while IFS='|' read -r score skill_name grade lines _refs; do
        if [ "$grade" = "C" ] || [ "$grade" = "C+" ]; then
          if [ "$lines" -gt 300 ]; then
            ...
            echo "- **$skill_name** ($score/140) - Review description and anti-patterns" >> "$AUDIT_DIR/analysis.md"
          fi
        fi
      done
    echo "" >> "$AUDIT_DIR/analysis.md"
...
**Framework:** skill-judge (9-dimension evaluation)
EOF
}
...
EOF
  ...
  sort -t '|' -k1,1nr "$TMP_DIR/rows.txt" |
    while IFS='|' read -r score skill_name grade _lines _refs; do
      case "$grade" in
        D|F)
          echo "### $skill_name (Score: $score/140, Grade: $grade) - CRITICAL" >> "$AUDIT_DIR/remediation-plan.md"
          echo "" >> "$AUDIT_DIR/remediation-plan.md"
          echo "Run: \`sh skills/agentic-harness/skill-quality-auditor/scripts/evaluate.sh $skill_name --json\` for details" >> "$AUDIT_DIR/remediation-plan.md"
          echo "" >> "$AUDIT_DIR/remediation-plan.md"
          ;;
      esac
    done
...
EOF
  ...
  sort -t '|' -k1,1nr "$TMP_DIR/rows.txt" |
    while IFS='|' read -r score skill_name grade _lines _refs; do
      case "$grade" in
        C|C+)
          echo "- **$skill_name** ($score/140) - Review D3 (anti-patterns) and D4 (description)" >> "$AUDIT_DIR/remediation-plan.md"
          ;;
      esac
    done
...
**Generated by:** skill-quality-auditor  
**Run with:** \`sh skills/agentic-harness/skill-quality-auditor/scripts/generate-remediation-plan.sh --audit-dir $AUDIT_DIR/\`
EOF
}
...
echo
echo "Summary:"
echo "  Average Score: $AVG_SCORE/140"
