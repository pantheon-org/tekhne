...
const workspace =
  workspaceIdx !== -1 ? args[workspaceIdx + 1] : findWorkspace();
...
async function extractAuthHook(): Promise<string> {
  const content = await Bun.file(PLUGIN_INDEX).text();
  const authHookMatch = content.match(
    /export type AuthHook = \{[\s\S]*?\n\}\n/m,
  );
  const authResultMatch = content.match(
    /export type AuthOuathResult[\s\S]*?\n\)\n/m,
  );
  return [authHookMatch?.[0] || "", authResultMatch?.[0] || ""].join("\n");
}

function generateHooksReference(
  hooks: string,
  pluginInput: string,
  authHook: string,
): string {
...
function generateEventsReference(
  events: EventInfo[],
  eventTypes: string[],
): string {
  const timestamp = new Date().toISOString();
...
async function main() {
  ...
  const [hooks, pluginInput, authHook, events, eventTypes, toolDef] =
    await Promise.all([
