...
//! `skills/documentation/adr-creator` tree into the binary as a table of
//! `(relative-path, bytes)` entries. This module exposes that table and
//! materialises it onto disk so a downloaded binary can install the skill on a
//! machine that has no copy of the repository.
...
/// The directory name of the embedded skill (e.g. `adr-creator`).
...
mod tests {
    ...
    fn bundle_is_non_empty() {
        assert!(file_count() > 0);
        assert_eq!(skill_name(), "adr-creator");
    }
    ...
    fn materialise_writes_the_tree() {
        ...
        assert_eq!(skill_dir, tmp.path().join("adr-creator"));
