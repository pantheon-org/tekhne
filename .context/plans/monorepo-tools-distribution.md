...
**Status**: ✅ **COMPLETE — all 7 waves merged to `main` (22-07-2026, `e8d6c81b`)**. GO/NO-GO gate returned GO. The Go toolchain and the TypeScript `cli/` are retired; six Rust crates (`common`, `skill-install`, `skill-validator-rs`, `skill-auditor`, `adr`, `journal`) with hooks/CI on build-from-source; Python narrowed to the allowlist with a guardrail; docs Tools section shipped; **all four tool crates released at `v0.1.0`** (installers + macOS/Linux binaries). Remaining items are the deferred follow-ups listed below (catalog test coverage, tessl bulk manage), none blocking. Release-please reconciliation and the Python-guardrail CI step completed 23-07-2026. See Definition of Done.
...
- **`wip/validator-quality-audit`** ([#244](https://github.com/pantheon-org/tekhne/issues/244)) — parked branch with grade-C improvements for the 12 validator/generator skills (D/F -> C/C+). Formerly blocked by the skill-artifacts name-vs-directory validator bug (#243); **now unblocked** since #243 is fixed.
- ~~**skill-artifacts name-vs-directory bug** ([#243](https://github.com/pantheon-org/tekhne/issues/243)) — the artifacts check false-positives on consolidated `<tool>-validator`/`-generator` skills; blocks #244.~~ **DONE 24-07-2026:** `check_skill_dir` now accepts the consolidated `<tool>/{generator,validator}` layout (`<parent>-<dir>` name form) alongside the bare directory name; regression tests cover a correctly-structured pair and a genuinely-wrong name. All 22 consolidated `SKILL.md` files validate clean.
...
- **D5, validators as Rust binaries (revises decision 1). RESOLVED: plain skills, not crates.** Validators wrap external linters or carry custom rule logic; neither fits the self-contained-binary model, and a Rust binary would still require the external tool installed. Two sub-categories, classified per tool in C2b: **(i) external-linter wrappers** (terraform, terragrunt, k8s/yaml-validator, helm, ansible, fluentbit-validator, k8s/debug) become bash-runner plain skills with a tool-presence check, Python removed; **(ii) custom-logic validators/generators** (promql best-practices, gitlab-ci, azure-pipelines, jenkinsfile-generator, fluentbit-generator) keep allowlisted Python, since their logic has no external tool to delegate to. Consequences: decision 1 reversed; C2b becomes a plain-skill conversion (moved to Wave 2, out of the Rust track); no cargo-dist validator crates; Wave 6 shrinks. **Revised 22-07-2026 (C2b execution): all 12 kept as allowlisted Python (no bash conversion) — the 7 "wrappers" carry real parser/validator logic.** Marking is via the root `python-allowlist.txt` only; per-skill frontmatter markers on the validators are blocked both by their D/F audit grades and by the skill-artifacts validator's name-vs-dir bug (both surfaced when touching `SKILL.md`). Grade-C quality fixes are parked on `wip/validator-quality-audit`.
...
- [ ] (Post-gate) Plain skills discoverable by skills.sh (convention discovery, no manifest, ~74 agents; supersedes the bespoke 42-agent installer per B1) and tessl (per-plugin, retained); no agent served by `cli/` loses an install path (verified in B1); every `cli/` command (audit, tessl, install, uninstall, readme, sync, validate) has a named successor owner. Root tessl/`skills.toml` manifests do not exist; Claude-marketplace/apm deferred pending a layout restructure (B1).
- [ ] (Post-gate) The TS `cli/` and JS `validate-schema` removed; every job has a named new owner.
- [ ] (Post-gate) Python removed **except an allowlist** covering the six research skills (D1, marked in SKILL.md frontmatter + the root allowlist) and the custom-logic validator/generator skills (D5, listed in the root `python-allowlist.txt` which is authoritative); no other `.py`, inline `python3`, pip config, or python CI matrix remains outside the allowlist.
...
- [x] **S1** — skill-validator spike: the validator is an external Go module (`agent-ecosystem/skill-validator@v1.5.6`, ~6.6k non-test LOC across ~14 packages). The deterministic gate subset the hooks need (`validate`/`check`/`structure`/`content`, excluding the `score`/`judge` LLM path) is ~3k non-test LOC. Scope A4a to that subset and explicitly exclude `score`/`judge`. Decide Go-free (reimplement, no wrapper). Produce a golden-corpus + score-tolerance spec for A4b. **Gates A4a/A4b.** — branch `spike/skill-validator-port` — model: smart — **DONE (commit `80492a87`): spec at `.context/plans/skill-validator-port-spec.md`; `o200k_base` token-parity go/no-go front-loaded as the A4a gate; external link-checking made opt-in (behaviour change for A6).**
- [x] **R1** — release-please × cargo-dist policy: decision doc (tag namespaces, version source of truth per package type, how relocated tool-backed skills leave `release-please-config.json` + manifest). **Also fix the stale extra-files (D4): repoint all 61 `tile.json` extra-files to `.tessl-plugin/plugin.json` `$.version` (or remove them), and reconcile any version drift between the manifest and the published `plugin.json`.** Consumed by A5/A6/A7/A8 — branch `docs/release-versioning-policy` — model: standard — **DONE (commit `9210b09d`): policy doc at `.context/plans/release-versioning-policy.md`; all 61 extra-files repointed to `.tessl-plugin/plugin.json`. Uncovered 100% manifest/plugin.json version drift and 6 stale husk entries (relocated/renamed skills); both flagged as reviewed follow-ups (outward-facing release changes), not executed.**
- [x] **B1** — Root ecosystem manifests (tessl `.tessl-plugin/plugin.json` `skills: ./skills/`, root `skills.toml`, `.claude-plugin/marketplace.json`, apm equivalent); verify each installer discovers `./skills` and that the 42 agents in `cli/lib/types/agents.ts` retain an install path (document gaps). Coordinate the root-vs-per-skill publish-unit model with B3 — branch `feat/root-skill-manifests` — model: standard — **DONE (commit `be5756dc`): premises corrected via verification. Root tessl plugin + `skills.toml` are not real; skills.sh convention-discovers our layout with no manifest and covers ~74 agents (superset of the 42), so no coverage is lost when `cli/` drops; tessl stays per-plugin; Claude-marketplace/apm deferred (layout mismatch). See `.context/plans/skill-distribution.md`.**
- [x] **B3** — tessl publish/manage as npm scripts + docs; reconcile with / replace `.github/workflows/publish-skills.yml` — branch `feat/tessl-scripts` — model: fast — **DONE (no code change; documented with B1 in `.context/plans/skill-distribution.md`, commit `be5756dc`): `publish-skills.yml` already uses the `tessl` CLI (not `cli/`), so it survives the CLI drop unchanged; only `tessl:manage` / `tessl:publish-check` are cli-coupled and are flagged for B5 to re-home or retire. No separate branch needed.**
- [x] **B4** — Drop `tools/validate-schema` (no runtime consumers) — branch `chore/drop-validate-schema` — model: fast — **DONE (no git change needed): already untracked. Its source and README were removed in earlier history (e.g. `325dd753`) when it was consolidated into the CLI; only a stale gitignored `dist/validate-schema.js` artifact remains in the working tree, with zero consumers (confirmed via grep across ts/js/json/pkl/yml + package.json). No branch/PR; recommend deleting the local stale artifact.**
- [x] **C1** — Authoritative Python + glue inventory (all `.py` incl. fluentbit validator+generator, helm/ansible validators, `k8s/debug/pod_diagnostics.py` distinct from `k8s/yaml-validator`, and jenkinsfile `generator/` scripts; `requirements.txt` (eight: six under research/, two vendored under `.agents/skills/`); inline `python3`; vendored Python skills; dependabot pip; codeql python matrix; `scripts/` glue). **Create the research-skill allowlist and add the frontmatter exemption marker** to the six research skills per D1: PubMed, Sci-Hub, notebooklm, sci-data-extractor, google-scholar-search, semantic-scholar-search. Confirm `.agents/skills/**` vendored copies (pubmed-search, sci-hub-search) are carved out of the C3 verification. Source of truth for C2/C2b/C3 — branch `chore/python-inventory` — model: standard — **DONE (commit `880920a3`): inventory at `.context/plans/python-inventory.md`; root `python-allowlist.txt` (11 skills: 6 research + 5 D5 custom-logic); 6 research SKILL.md markers added and validated. Category-(i) wrapper markers/conversion deferred to C2b.**
...
- [x] **A2** — `common` crate: config loading, error types, structured output, fs helpers (+ tests) — branch `feat/common-crate` — model: standard — **DONE (#179): 4 modules (error/config/output/fs), 11 unit tests, fmt/clippy/build/test green.**
- [x] **B2a** — Astro Skills section from `./skills`, plus a replacement for the root `README.md` generation done today by `cli/lib/readme` (~1.7k LOC) — branch `feat/docs-skills-and-readme` — model: standard — **DONE: relocated the 1689-LOC generator `cli/lib/readme` -> `scripts/catalog/` (standalone Bun `update.ts`, inlined logger, decoupled from Cliffy); repointed `package.json` readme scripts + `hk.pkl` readme-sync; `cli/commands/readme.ts` delegates to it (kept green until B5). Astro Skills section already docs-owned via `prelink.mjs` + dynamic skill/tile pages (no new work). 311 unit + 28 cucumber pass. Follow-up: the generator still discovers `tile.json` (removed in PR #93) so the catalog shows 0 tiles — repoint to `.tessl-plugin/plugin.json` (relates to R1).**
- [x] **C2** — Drop dead Python; move thin glue to Bun per C1; replace the inline `python3` JSON parse in `hk.pkl` (:97) and `skill-audit.yml` (:70-71) with `jq`, and **add `jq` to `mise.toml`** (only `yq` is pinned today) so the audit gate stays behaviour-preserving on contributor machines. Do not touch allowlisted skills — branch `chore/python-glue-and-drop` — model: standard — **DONE (#180): inline python3 -> jq in hk.pkl + skill-audit.yml (behaviour verified identical); jq pinned in mise.toml; no dead Python existed to drop (C1).**
- [x] **C2b** — Validators to plain skills per D5 — **done via root allowlist only (two blockers surfaced).** Inspection showed the 7 "external-linter wrappers" carry substantial custom Python (HCL/YAML parsers, a 1171-LOC fluentbit validator; external-tool calls already in `.sh`), so **all 12 validator/generator skills are kept as allowlisted Python — no bash conversion.** Adding per-skill frontmatter markers was blocked twice by touching `SKILL.md`: (1) all 12 grade D/F on the current auditor, tripping the required Skill Audit gate; (2) the pre-commit skill-artifacts validator's naive name-vs-directory rule flags every `<tool>-validator/-generator` skill (apparent validator bug). Resolution: the root `python-allowlist.txt` (18 skills) is authoritative; no `SKILL.md` touched. Grade-C quality improvements for all 12 (D/F->C/C+) are parked on `wip/validator-quality-audit` pending a validator-name-rule fix — branch `chore/validators-to-plain-skills` — model: standard
...
- [x] **A6** — **DONE (PR #198, `6daa79f1`):** Go toolchain retired. Hooks/CI/docs repointed to build-from-source Rust (`cargo run/build -p skill-auditor` / `-p skill-validator-rs`); `skill-audit.yml` keeps the required "Skill Audit" check name; `go` removed from CodeQL; `gomod` dependabot replaced with a `cargo` ecosystem entry; `go`+`skill-validator` mise pins removed; `tools/skill-auditor/` (53 files) + `skill-validate.yml` deleted (`bin/skill-auditor` was gitignored). Rollback tag `pre-a6-retire-go`. Original task text below. — Retire the Go toolchain **[must land together]**: (a) repoint `hk.pkl` (incl. removing the now-dead `go-mod-tidy` step that globs `tools/skill-auditor/**/*.go`), `package.json`, `.github/workflows/{skill-audit,codeql,pr-skill-checks}.yml`, `dependabot.yml` (gomod), `README.md`, `AGENTS.md`, SKILL.md to the Rust auditor + `skill-validator-rs`, mise installing them for CI; in `codeql.yml` remove the `go` matrix entry, the `setup-go` step, the `go build` step, and the `tools/skill-auditor/go.sum` cache path; (b) delete `tools/skill-auditor/`, `bin/skill-auditor`, `.github/workflows/skill-validate.yml`, and the `go` + `skill-validator` pins from `mise.toml`. Requires A6c to land in lockstep. Merge atomically — branch `chore/retire-go` — model: standard
- [x] **A6c** — **DONE (with A6 #198):** Rust static-analysis story = clippy (already gating in `rust-ci.yml`) + new `cargo-audit` step (RUSTSEC; clean, 0 advisories; `continue-on-error` first-pass). Branch-protection: only "Skill Audit" is a required status check (confirmed), so deleting `skill-validate.yml` needed no status-check remap. The `code_scanning` "go configuration not found" block (which even `--admin` cannot bypass) was cleared via a **ruleset-window**: temporarily removed the `code_scanning` rule, merged A6, waited for `main`'s js/py CodeQL to complete, then re-added the identical rule (backup at scratchpad `ruleset-main-backup.json`). Gate restored. The same ruleset-window will be needed for C3 (python leaving). Original text below. — Branch-protection / CodeQL ruleset update **[lands with A6, and again with C3]**: when `go` (A6) and later `python` (C3) leave the CodeQL matrix, update `main`'s `code_scanning` ruleset so PRs are not blocked by "configurations not found" (per PR #126); when `skill-validate.yml` is deleted (A6), re-map the required status checks so "Skill Validator" is not left permanently unreported. Also decide the Rust static-analysis story (CodeQL has no Rust support) — branch `chore/codeql-ruleset` — model: standard
- [x] **A7** — `adr` crate CLI with bundled skill + installer via the A5 template — branch `feat/adr-crate` — model: standard — **DONE (PR #195): net-new `adr` CLI (`new`/`list`/`supersede` + `skill install`) using the house ADR template; new first-class `adr-creator` skill (audited B+ 121/140) embedded from canonical path; 27 tests. This PR also ran `dist generate` to add all three tools (skill-auditor, journal, adr) to `tool-release.yml` and set `pr-run-mode = "skip"` so PRs no longer hang at UNSTABLE. Follow-up: give `adr-creator` a release-please entry (deferred R1 reconciliation).**
- [x] **A8** — `journal` crate CLI with bundled skill + installer via the A5 template (promoted from plain skill per decision 7) — branch `feat/journal-crate` — model: standard — **DONE (PR #194): `journal` CLI (`new --type` across the 5 template schemas, `validate` porting `validate-journal-entry.sh`, `skill install`); 41 tests; output verified 5/5 against the original shell validator. `journal-entry-creator` skill embedded from canonical path and kept first-class in `skills/`.**
- [x] **B5** — **DONE (PR #202, `85c3cc68`):** TS `cli/` removed + `pr-cli-tests.yml` retired. New `skill uninstall` verb (`skill-install::uninstall_skill`) on skill-auditor/adr/journal. Repointed package.json (`test`→`bun test scripts/`, `audit:skill`→Rust auditor, sync/tessl-manage scripts dropped, dead `@cliffy/*` pruned), hk.pkl (frontmatter step dropped as redundant with `skill-validate`, ts-conventions dropped, unit-tests→`bun test scripts/`), docs/README/CONTRIBUTING/AGENTS. Cucumber suite relocated to top-level `features/`: readme→`scripts/catalog`, validate→`skill-validator-rs`; sync/audit/install/tessl removed with rationale (4 scenarios/14 steps green). `cargo test --workspace` + `bun test scripts/` (101) + cucumber green. Follow-ups: `skill-install/tests/parity.rs` removed (embedded the deleted TS table; covered by in-crate tests); tessl bulk `manage`/`publish-check` left ownerless (pre-existing gap). Original text below. — Drop TS `cli/` **[must land together]**: remove `cli/` + repoint `hk.pkl`, `package.json`, `cucumber.js`, `docs/src/pages/index.astro` (incl. the hardcoded install demo at :68), `README.md`, `CONTRIBUTING.md`, `AGENTS.md`; retire `pr-cli-tests.yml`. Owners for all 7 commands (resolved 22-07-2026): install fan-out → ecosystem installers + tool `skill install`; **uninstall → new `skill uninstall` verb on the tool `skill install` subcommand** (net-new in the `skill-install` crate); frontmatter validation → `skill-validator-rs` (build-from-source, A5b binary); README generation → B2a (`scripts/catalog`); bulk audit → `skill-auditor` CLI; tessl → B3; **sync (sync opencode) → RETIRE with rationale** (repo-local dev helper, no distributed consumer; delete `cli/lib/sync/*`, the command, the two npm scripts, and `sync.feature`). **Port/replace the `cli/features/*.feature` cucumber suite** (audit, install, readme, sync, tessl, validate) against new owners; note several owners are external installers, so those tests may become network-dependent. Merge atomically — branch `chore/drop-ts-cli` — model: standard
...
Verification:
- [x] Hooks/CI use only Rust tools; grep shows no Go `skill-auditor`/`skill-validator` or `bun cli`/`cli/index.ts` outside history — verified.
- [x] `curl | sh` installs `adr` and `journal` binaries + `skill install` registers their skills — `dist=true` + `tool/`-namespaced release workflow; clean-machine install path proven for the skill-auditor pattern the crates share. (First actual release tag not yet cut.)
- [x] Acceptance coverage for audit/install/readme/sync/tessl/validate ported to the new owners — `readme`→`scripts/catalog`, `validate`→`skill-validator-rs` (cucumber, green); `sync` retired; `audit`/`install`/`tessl` covered by cargo tests or removed with rationale (external/B3 owners, no offline gate).
...
- [x] `skill-auditor`, `adr`, and `journal` install standalone (macOS/Linux) and register their skills via `skill install`; validators ship as plain skills (D5) — **first releases cut at `v0.1.0` for all four tool crates (skill-auditor, adr, journal, skill-validator-rs), each with the shell installer + 4 platform tarballs (both Linux + both macOS) + checksums.** Cutting the first tag surfaced two stale-runner-label bugs in cargo-dist 0.28's generated workflow, fixed via `[dist.github-custom-runners]`: `ubuntu-20.04` (retired) -> `ubuntu-22.04` (#207) and `macos-13` (not in the org's runner set) -> `macos-latest` (#208). The release workflow is now proven end-to-end across all four targets.
- [x] release-please and cargo-dist coexist under the R1 policy — **coexistence yes** (disjoint `tool/` vs `v` tag namespaces, disjoint triggers); **"no stale entries" now met (23-07-2026)**: the R1 version-drift reconciliation ran (max/no-regression across all packages), the 6 stale entries were already gone, and all 17 unmanaged skills (incl. `adr-creator`) were added, so config and manifest hold identical 74-package key-sets with zero drift. See `release-versioning-policy.md` section 6.
...
**Deferred follow-ups (non-blocking, tracked in "Open follow-ups"):** reconcile the stale C1-inventory wording; re-home tessl bulk `manage`/`publish-check`; `scripts/catalog` test coverage re-wiring; `tile.json` discovery repoint. (Python guardrail promoted to a CI step 23-07-2026 via the Python Allowlist workflow; R1 release-please version-drift + entry reconciliation done 23-07-2026; first `tool/*-v*` releases were cut at v0.1.0 - done.)
