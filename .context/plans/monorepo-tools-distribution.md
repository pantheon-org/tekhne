...
- **`adr-creator` release-please entry** — the new A7 skill is not yet a release-please package; add it (part of the deferred R1 reconciliation) so it is version-managed like other skills.
...
**Deferred follow-ups (non-blocking, tracked in "Open follow-ups"):** R1 release-please version-drift + stale-entry reconciliation; `adr-creator` release-please entry; promote the Python guardrail to a CI step; reconcile the stale C1-inventory wording; re-home tessl bulk `manage`/`publish-check`; `scripts/catalog` test coverage re-wiring; `tile.json` discovery repoint. (First `tool/*-v*` releases were cut at v0.1.0 - done.)
