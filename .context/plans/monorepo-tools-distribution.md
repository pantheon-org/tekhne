...
**Deferred follow-ups (non-blocking, tracked in "Open follow-ups"):** promote the Python guardrail to a CI step; reconcile the stale C1-inventory wording; re-home tessl bulk `manage`/`publish-check`; `scripts/catalog` test coverage re-wiring; `tile.json` discovery repoint. (R1 release-please version-drift + entry reconciliation done 23-07-2026; first `tool/*-v*` releases were cut at v0.1.0 - done.)
